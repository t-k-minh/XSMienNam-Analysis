...
function runBacktest() {{
    ...
    const windows = [30, 90, 180, 365, 0]; // 0 = all data
    const windowLabels = ['30 ngày', '3 tháng', '6 tháng', '1 năm', 'Tất cả'];

    // For each province, test predictions
    ...
    allProvinces.forEach(prov => {{
        ...
        if (provData.length < 50) return; // Skip if not enough data
        ...
        // Random baseline: expected accuracy = 6/100 = 6% for 2-digit numbers
        const randomAcc = (6/100*100).toFixed(1);

        windows.forEach((winDays, wi) => {{
            ...
            for (let i = trainEnd; i < provData.length; i++) {{
                ...
                // Get training data before this date
                ...
                // Calculate scores and get top 6
                ...
                for (let k in actual) {{
                    if ((k.startsWith('prize') || k === 'special') && actual[k]) {{
                        const n = parseInt(String(actual[k]).slice(-2));
                        if (!isNaN(n)) actualNums.add(n);
                    }}
                }}

                // Count matches
                predicted.forEach(n => {{ if (actualNums.has(n)) correct++; }});
                total++;
            }}

            const acc = total > 0 ? (correct / total * 100).toFixed(1) : '0.0';
            const color = parseFloat(acc) > 15 ? '#27ae60' : parseFloat(acc) > 10 ? '#f39c12' : '#e74c3c';
            row += '<td style="padding:8px;text-align:center;font-weight:700;color:' + color + '">' + acc + '%</td>';
        }});

        row += '<td style="padding:8px;text-align:center;color:#999">' + randomAcc + '%</td>';
        row += '</tr>';
        html += row;
    }});
    ...
    html += '<td style="padding:8px" colspan="2">Trung bình tất cả tỉnh</td>';
    ...
    windows.forEach((winDays, wi) => {{
        ...
        allProvinces.forEach(prov => {{
            ...
            for (let i = trainEnd; i < provData.length; i++) {{
                ...
                const predicted = new Set(r.sorted.slice(0,6).map(([n])=>parseInt(n)));
                const actualNums = new Set();
                for (let k in provData[i]) {{
                    if ((k.startsWith('prize') || k === 'special') && provData[i][k]) {{
                        const n = parseInt(String(provData[i][k]).slice(-2));
                        if (!isNaN(n)) actualNums.add(n);
                    }}
                }}
                predicted.forEach(n => {{ if (actualNums.has(n)) correct++; }});
            ...
            if (total > 0) {{ totalAcc += correct/total*100; count++; }}
        }});
        const avg = count > 0 ? (totalAcc/count).toFixed(1) : '0.0';
    ...
    html += '<p><b>Cách đọc:</b> Mỗi ô hiển thị % trúng trong top 6 dự đoán so với kết quả thực tế.</p>';
    html += '<p><b>Ngẫu nhiên:</b> 6.0% là xác suất ngẫu nhiên chọn 6 số đúng từ 100 số (00-99).</p>';
    html += '<p><b>Màu sắc:</b> <span style="color:#27ae60">Xanh > 15%</span> | <span style="color:#f39c12">Vàng > 10%</span> | <span style="color:#e74c3c">Đỏ ≤ 10%</span></p>';
    html += '<p><b>Kết luận:</b> Nếu % > 6% liên tục → thuật toán cóedge so với ngẫu nhiên. Nếu % ≈ 6% → không khác ngẫu nhiên.</p>';
    html += '</div>';
